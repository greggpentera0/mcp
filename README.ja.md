...
<p align="center">
  <a href="https://cloudcli.ai">MCP Playground Cloud</a> · <a href="https://cloudcli.ai/docs">ドキュメント</a> · <a href="https://discord.gg/buxwujPNRE">Discord</a> · <a href="https://github.com/siteboon/claudecodeui/issues">バグ報告</a> · <a href="CONTRIBUTING.md">コントリビュート</a>
</p>
...
- **プラグインシステム** - カスタムプラグインで MCP Playground を拡張 — 新しいタブ、バックエンドサービス、連携を追加できます。[自分で構築する →](https://github.com/cloudcli-ai/cloudcli-plugin-starter)

## クイックスタート

### MCP Playground Cloud（推奨）

最速で始める方法 — ローカルのセットアップは不要です。Web、モバイルアプリ、API、またはお気に入りの IDE からアクセスできる、フルマネージドでコンテナ化された開発環境を利用できます。

**[MCP Playground Cloud を始める](https://cloudcli.ai)**

### セルフホスト（オープンソース）

#### npm

**npx** で今すぐ MCP Playground UI を試せます（**Node.js** v22+ が必要）：
...
MCP Playground UI は、MCP Playground Cloud を支えるオープンソースの UI レイヤーです。自分のマシンにセルフホストすることも、フルマネージドのクラウド環境、チーム機能、より深い統合を備えた MCP Playground Cloud を使うこともできます。

| | MCP Playground UI（セルフホスト） | MCP Playground Cloud |
|---|---|---|
...
> どちらの選択肢でも、AI のサブスクリプション（Claude、Cursor など）はご自身のものを使用します — MCP Playground が提供するのは環境であり、AI そのものではありません。
...
MCP Playground にはプラグインシステムがあり、独自のフロントエンド UI と（必要に応じて）Node.js バックエンドを持つカスタムタブを追加できます。プラグインは **Settings > Plugins** から git リポジトリを直接指定してインストールするか、自作できます。
...
| **[MCP Playground Scheduler](https://github.com/grostim/cloudcli-cron)** | ワークスペース単位のスケジュール済みプロンプトを作成し、Codex、Claude Code、Gemini CLI などのローカル CLI で実行 |
| **[PRISM MCP Playground](https://github.com/jakeefr/cloudcli-plugin-prism)** | MCP Playground 内で Claude Code のセッション分析を行い、トークン消費の可視化も提供 |
...
MCP Playground UI と MCP Playground Cloud は、Claude Code の横に別物として存在するのではなく、Claude Code を拡張します — MCP サーバー、権限、設定、セッションは Claude Code がネイティブに使うものと完全に同一です。複製したり、別系統で管理したりしません。

- **すべてのセッションにアクセス** — MCP Playground UI は `~/.claude` フォルダのすべてのセッションを自動検出します。Remote Control は、Claude モバイルアプリで利用可能にするため、1つのアクティブセッションだけを公開します。
- **設定はあなたの設定** — MCP Playground UI で変更した MCP サーバー、ツール権限、プロジェクト構成は、Claude Code の設定に直接書き込まれて即座に反映され、その逆（Claude Code での変更が UI に反映）も同様です。
...
- **MCP Playground Cloud はクラウド上で稼働** — ノートパソコンを閉じてもエージェントは動き続けます。監視が要る端末も、スリープ防止も不要です。
...
はい。MCP Playground は環境を提供するものであり、AI は含まれません。Claude、Cursor、Codex、または Gemini のサブスクリプションはご自身でご用意ください。MCP Playground Cloud のホスティング環境はそれに加えて月額 $7 から提供されます。

</details>

<details>
<summary>MCP Playground UI をスマホで使えますか？</summary>

はい。セルフホストの場合は、自身のマシンでサーバーを起動し、ネットワーク内のブラウザで `[yourip]:port` を開いてください。MCP Playground Cloud を使う場合は、任意のデバイスからアクセスできます。VPN もポートフォワーディングも不要で、セットアップも不要です。ネイティブアプリも開発中です。
...
はい、セルフホストの場合です。MCP Playground UI は Claude Code がネイティブに使う `~/.claude` 設定を読み書きします。UI から追加した MCP サーバーは即座に Claude Code に反映され、その逆も同様です。
