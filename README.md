...
OpenCode local models can come from `~/opencode.json`, `~/.config/opencode/opencode.json`, or `~/.config/opencode/opencode.jsonc`. Ollama-backed IDs such as `ollama/qwen3-coder:latest` are read from that config and shown in the model picker.
