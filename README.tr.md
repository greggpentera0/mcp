...
<p align="center">
  <a href="https://cloudcli.ai">MCP Playground Cloud</a> · <a href="https://cloudcli.ai/docs">Dokümantasyon</a> · <a href="https://discord.gg/buxwujPNRE">Discord</a> · <a href="https://github.com/siteboon/claudecodeui/issues">Sorun Bildir</a> · <a href="CONTRIBUTING.md">Katkıda Bulun</a>
</p>

<p align="center">
  <a href="https://cloudcli.ai"><img src="https://img.shields.io/badge/☁️_MCP Playground_Cloud-Hemen_Dene-0066FF?style=for-the-badge" alt="MCP Playground Cloud"></a>
...
- **Eklenti Sistemi** — MCP Playground'ı özel eklentilerle genişlet: yeni sekmeler, arka uç servisleri ve entegrasyonlar ekle. [Kendi eklentini yaz →](https://github.com/cloudcli-ai/cloudcli-plugin-starter)
...
**[MCP Playground Cloud ile başla](https://cloudcli.ai)**
...
MCP Playground UI'yi **npx** ile anında dene (**Node.js** v22+ gerekir):
...
MCP Playground UI, MCP Playground Cloud'u güçlendiren açık kaynak arayüz katmanıdır. Kendi makinende barındırabilir, izolasyon için Docker sandbox'ta çalıştırabilir veya tam yönetilen ortam için MCP Playground Cloud kullanabilirsin.

| | Kendin Barındır (npm) | Kendin Barındır (Docker Sandbox) *(Deneysel)* | MCP Playground Cloud |
|---|---|---|---|
...
> Tüm seçenekler kendi AI aboneliklerini (Claude, Cursor, vb.) kullanır — MCP Playground AI'ı değil, ortamı sağlar.
...
MCP Playground, kendi frontend UI'sı ve isteğe bağlı Node.js arka ucu olan özel sekmeler eklemeni sağlayan bir eklenti sistemine sahiptir. Git depolarından eklentileri doğrudan **Ayarlar > Eklentiler**'den yükleyebilir veya kendi eklentini yazabilirsin.
...
| **[MCP Playground Scheduler](https://github.com/grostim/cloudcli-cron)** | Çalışma alanı kapsamlı zamanlanmış prompt'lar oluşturur ve bunları Codex, Claude Code veya Gemini CLI gibi yerel CLI'larla çalıştırır |
| **[PRISM MCP Playground](https://github.com/jakeefr/cloudcli-plugin-prism)** | MCP Playground içinde Claude Code oturum zekası ve token tüketimi görünürlüğü sağlar |
...
MCP Playground UI ve MCP Playground Cloud, Claude Code'un yanında değil içinde çalışır — MCP sunucuların, izinlerin, ayarların ve oturumların, Claude Code'un yerel olarak kullandığının birebir aynısıdır. Hiçbir şey çoğaltılmaz veya ayrı yönetilmez.

Pratikte bu ne demek:

- **Tek oturum değil, tüm oturumların** — MCP Playground UI, `~/.claude` klasöründeki her oturumu otomatik keşfeder. Remote Control yalnızca tek aktif oturumu Claude mobil uygulamasına açar.
...
- **MCP Playground Cloud bulutta çalışır** — laptop'unu kapat, ajan çalışmaya devam eder. Beklemen gereken terminal yok, uyanık tutman gereken makine yok.
...
Evet. MCP Playground AI'yi değil, ortamı sağlar. Kendi Claude, Cursor, Codex veya Gemini aboneliğini getirirsin. MCP Playground Cloud, barındırılan ortam için aylık 7 $'dan başlar — bunun üzerine eklenir.

</details>

<details>
<summary>MCP Playground UI'yi telefonumda kullanabilir miyim?</summary>

Evet. Kendin barındırdığında, sunucuyu makinende çalıştır ve ağındaki herhangi bir tarayıcıda `[yourip]:port` adresini aç. MCP Playground Cloud için, herhangi bir cihazdan aç — VPN yok, port yönlendirme yok, kurulum yok. Native bir uygulama da hazırlanıyor.
...
Evet, kendin barındırdığında. MCP Playground UI, Claude Code'un yerel olarak kullandığı aynı `~/.claude` yapılandırmasından okur ve ona yazar. UI üzerinden eklediğin MCP sunucuları Claude Code'da anında görünür; tersi de geçerli.
